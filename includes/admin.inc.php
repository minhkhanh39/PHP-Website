...
if (isset($_POST["submit"]))
{
  ...
  if ($util->EmptyInputCreateUser($username, $pass, $repeatPass, $email, $privilegeLevel))
  {
    ...
    echo "<script>document.getElementById('message').innerHTML = '*Điền vào tất cả các trường!';</script>";
    exit();
  }
  if ($util->pwdNotMatch($pass, $repeatPass))
  {
    echo "<script>document.getElementById('message').className = 'errormsg';</script>";
    echo "<script>document.getElementById('message').innerHTML = '*Mật khẩu không khớp!';</script>";
    exit();
  }
  if ($util->invalidUid($username))
  {
    echo "<script>document.getElementById('message').className = 'errormsg';</script>";
    echo "<script>document.getElementById('message').innerHTML = '*Chọn tên người dùng thích hợp!';</script>";
    exit();
  }
  if (uidExists($username, $email, $util))
  {
    echo "<script>document.getElementById('message').className = 'errormsg';</script>";
    echo "<script>document.getElementById('message').innerHTML = '*Tên người dùng đã được sử dụng!';</script>";
    exit();
  }
  ...
  echo "<script>document.getElementById('message').innerHTML = 'Đã thêm người dùng.';</script>";
}
...
if (isset($_POST["submit_product"]))
{
  ...
  if ($util->productExists($image)) {
    echo "<script>document.getElementById('message').className = 'errormsg';</script>";
    echo "<script>document.getElementById('message').innerHTML = '*Sản phẩm tồn tại! Hãy thử một hình ảnh khác.';</script>";
    exit();
  }
  ...
  echo "<script>document.getElementById('message').className = 'green-text';</script>";
  echo "<script>document.getElementById('message').innerHTML = 'Đã thêm sản phẩm.';</script>";
}
