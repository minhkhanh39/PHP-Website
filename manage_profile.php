...
<head>
  ...
  <title>Minh Khánh - Quản lý tài khoản</title>
  <?php include "header.php"; ?>
</head>
<body>
<h3 class="page-title grid" style="margin-top: 50px;">Quản lý hồ sơ cá nhân</h3>
  <div class="grid">
            ...
            <?php
              if (isset($_GET["error"]))
              {
                if ($_GET["error"] == "empty_input")
                  echo "<script>document.getElementById('msg').innerHTML = '* Điền vào tất cả các chỗ trống!';</script>";

                else if ($_GET["error"] == "invalid_uid")
                  echo "<script>document.getElementById('msg').innerHTML = '* Chọn tên người dùng thích hợp!';</script>";

                else if ($_GET["error"] == "passwords_dont_match")
                  echo "<script>document.getElementById('msg').innerHTML = '*Mật khẩu không khớp!';</script>";

                else if ($_GET["error"] == "stmtfailed")
                  echo "<script>document.getElementById('msg').innerHTML = '*Đã xảy ra lỗi. Vui lòng thử lại!';</script>";

                else if ($_GET["error"] == "username_taken")
                  echo "<script>document.getElementById('msg').innerHTML = '*Tên người dùng đã được sử dụng!';</script>";

                else if ($_GET["error"] == "none")
                {
                  echo "<script>document.getElementById('msg').className = 'green-text';</script>";
                  echo "<script>document.getElementById('msg').innerHTML = 'Hồ sơ cá nhân đã cập nhật!';</script>";
                }
              }
          ...
          <form class="s12" action="includes/manage_profile.inc.php" method="POST">
            <div class="row">
              <div class="input-field s6">
                ...
                ?>
                <label class='cyan-text' for="username">Nhập tên người dùng mới</label>
                <span class="helper-text grey-text" data-error="Min 5, Max 12 characters" data-success="Min 5, Max 12 characters">Tối thiểu 5, Tối đa 12 ký tự</span>
              </div>
            </div>
            <div class="row">
              <div class="input-field s6">
                ...
                ?>
                <label class='cyan-text' for="email">Nhập email mới</label>
            ...
            <div class="row">
              <div class="input-field s6">
                ...
                <label class='cyan-text' for="pwd">Nhập mật khẩu mới</label>
                <span class="helper-text grey-text" data-error="Min 8, Max 20 characters" data-success="Min 8, Max 20 characters">Tối thiểu 8, Tối đa 20 ký tự</span>
              </div>
            </div>
            <div class="row">
              <div class="input-field s6">
                ...
                <label class='cyan-text' for="repeat_pwd"> Lặp lại mật khẩu mới</label>
              </div>
            </div>
          <br>
          <p class="center-align">
          <button disabled id="update_acc" type="submit" name="update" class="btn orange darken-4">Cập nhật tài khoản</button>
          </p>
          </form>
        </div>        
      </div>
