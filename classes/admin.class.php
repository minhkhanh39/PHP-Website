...
class Admin extends Dbhandler{
        ...
        <tr>
          <td id='bordershadow'>
            <a href='product.php?item_id=$itemID'>
              ...
              <div class='white-text bold' style='height: 60px; float: left; margin-left: 150px'>Bình luận mới về $ratingDateTime</div>
  ...
  protected function searchMember(){
    ...
    if (isset($_POST["search_member"]))
    {
      ...
      if ($util->EmptyInputSelect($searchMember))
        echo "<p style='color: yellow'>Vui lòng nhập một giá trị</p>";
      else
      {
        ...
        $result = $this->conn()->query($sql) or die ("Người dùng không tồn tại!");
        while ($row = $result->fetch_assoc() ) 
        { 
  ...
  protected function inspectUser(){
    ...
    $result = $this->conn()->query($sql) or die ("Chọn câu lệnh THẤT BẠI!");
  ...
  protected function showProduct(){
    if (isset($_POST["search_product"]))
    {
      ...
      if ($emptyInput->EmptyInputSelect($searchProduct))
        echo "<p class='prompt-warning'>Vui lòng nhập một giá trị</p>";
      else
      {
        // limited search to prevent page overflow
        ...
        $result = $this->conn()->query($sql) or die ("Sản phẩm không tồn tại!");
        while ($row = $result->fetch_assoc() ) 
        {
          $itemID = $row["ItemID"]; 
          $name = $row["Name"];
      ...
      echo(
        "<tr>
          ...
          <td><a>
            ...
            onclick=\"return confirm('Bạn có chắc chắn muốn xóa bản ghi: \'$name, $brand\'?');\">Delete</button>
          </a></td>
        </tr>"
      );
    }
