...
class CommonUtil extends Dbhandler{
  ...
  public function setUser($username, $pwd, $email, $privilegeLevel=0, $attempt=3) {
    ...
    $this->conn()->query($sql) or die("<p>*Lỗi tạo người dùng, vui lòng thử lại!</p>");

    // get member id
    $sql = "SELECT MemberID FROM Members where Username = '$username';";
    $result = $this->conn()->query($sql) or die("<p>*Lỗi MemberID, vui lòng thử lại!</p>");
    ...
    $result = $this->conn()->query($sql) or die("<p>*Lỗi tạo giỏ hàng, vui lòng thử lại!</p>");
    $this->conn()->close();
  }
  ...
  public function setProduct($name, $brand, $description, $category, $sellingprice, $quantityinstock, $image)
  {
    ...
    $this->conn()->query($sql) or die("<p>*Lỗi tạo sản phẩm, vui lòng thử lại!</p>");
  }
